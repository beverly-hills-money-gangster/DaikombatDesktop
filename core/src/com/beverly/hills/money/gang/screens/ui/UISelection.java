...
public class UISelection<T extends Enum> {
    ...
    public void render(final BitmapFont guiFont64,
        ...
        for (int i = 0; i < this.getSelections().size(); i++) {
            ...
            final float optionY = gameScreen.getViewport().getWorldHeight() / 2f - optionGlyph.height / 2f - indent;
            guiFont64.draw(gameScreen.getGame().getBatch(), mainMenuUISelection.toString(), optionX, optionY);
